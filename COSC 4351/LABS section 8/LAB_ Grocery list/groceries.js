...
// Load the grocery list from localStorage
function loadList() {
   let groceryListString = localStorage.getItem("list");
   if (groceryListString) {
      return groceryListString.split(",");
   } else {
      return [];
   }
}
...
function saveList(groceryList) {
   let groceryListString = groceryList.join(",");
   localStorage.setItem("list", groceryListString);
}
